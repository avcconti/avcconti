# Andrés Velasco

**Engineering Manager** en [Product Hackers](https://producthackers.com) · Toledo, España

## Side projects

### 🎮 Rank Your Games

Rankea tu biblioteca de videojuegos comparándolos de dos en dos. Sin puntuar del 1 al 10:
eliges cuál prefieres y un sistema de rating Elo ordena el resto.

- Comparación por pares con rating Elo — el ranking emerge de decisiones simples
- Catálogo completo vía IGDB (Internet Game Database)
- Sincronización entre dispositivos con Supabase

[**App Store**](https://apps.apple.com/app/id6755431111) · [**rankyourgames.app**](https://www.rankyourgames.app)

### Con qué los construyo

![Swift](https://img.shields.io/badge/Swift-F05138?style=flat&logo=swift&logoColor=white)
![SwiftUI](https://img.shields.io/badge/SwiftUI-0071E3?style=flat&logo=swift&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3FCF8E?style=flat&logo=supabase&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-087EA4?style=flat&logo=react&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-5FA04E?style=flat&logo=nodedotjs&logoColor=white)
![Claude Code](https://img.shields.io/badge/Claude%20Code-D97757?style=flat&logo=claude&logoColor=white)

---

*Todo lo de este perfil es tiempo personal, no trabajo de Product Hackers.*

*Contacto profesional: [LinkedIn](https://www.linkedin.com/in/avelasco/) · Soporte de las apps: [info@avelasco.dev](mailto:info@avelasco.dev)*
